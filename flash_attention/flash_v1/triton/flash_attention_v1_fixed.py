...
def _flash_attention_kernel(
    ...
    l_i = tl.zeros([BLOCK_SIZE_M], dtype=tl.float32)  # 用于softmax归一化
    ...
    for k_col in range(0, tl.cdiv(seq_len, BLOCK_SIZE_N)):
        ...
        # 加载K、V块
        k_block_ptr = k_ptr + batch_offset + col_offsets_k[:, None] * HEAD_DIM + head_offsets[None, :]
        k_block = tl.load(k_block_ptr, mask=col_mask_k[:, None], other=0.0)
        
        v_block_ptr = v_ptr + batch_offset + col_offsets_k[:, None] * HEAD_DIM + head_offsets[None, :]
